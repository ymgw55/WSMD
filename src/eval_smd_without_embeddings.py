...
from modules.ot_utils import (
    rm_stopwords, stanza2dic_sent, tokens2words, clean_tokens)
...
def ot_method(method, rm_sw, weight_name, cost_name, params,
    ...
    if task == 'STSB':
        ...
        from datasets.sts12_16 import (
            STS12, STS13, STS14, STS15, STS16, data_all_set)
        task_func = eval(task)
    elif task == 'PAWSWiki':
        if full:
...
def main(model_name, emb_layer=None,
    ...
    if full:
        output_dir = root_dir /\
            f'output/full_eval_smd_without_embeddings/{task}/{dataset}'
    else:
        output_dir = root_dir /\
            f'output/eval_smd_without_embeddings/{task}/{dataset}'
    ...
    for weight_name in weight_names:
        if weight_name not in ('uniform', 'idf'):
            continue
